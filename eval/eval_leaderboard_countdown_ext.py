...
if __name__ == "__main__":
    ...
    for item in tqdm(data):
        ...
        gt = get_answer({"target": target, "nums": numbers})

        prompt = f"<|im_start|>user\nUsing the numbers {numbers}, create an equation that equals {target}. You can use basic arithmetic operations (+, -, *, /) and each number can only be used once. Show your work in <think> </think> tags. And return the final answer in <answer> </answer> tags, for example <answer> (1 + 2) / 3 </answer>. The correct answer is: <answer>{gt}</answer>. <|im_end|>\n"
        prompt += f"<|im_start|>assistant\n"
        response = generate_response(llm, prompt, temperature=0, top_p=1, max_tokens=5000, repetition_penalty=1.05)
